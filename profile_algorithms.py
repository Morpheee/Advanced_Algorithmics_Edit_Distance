    ...
    with Pool(processes=1) as pool:
        for file_name in file_list:
            ...
            for index,row in df.iterrows():
                ...
                results, duration_time, duration_clock = meassure_single_call_timeout(divide_and_conquer_ed,(row['string_1'],row['string_2']),timeout_time, pool)
                ...
                results, duration_time, duration_clock = meassure_single_call_timeout(ed_dp,(row['string_1'],row['string_2']),timeout_time, pool)
                if isinstance(results, int):
                    ed = results
                else:
                    ed = results['ed']
                ...
                results, duration_time, duration_clock = meassure_single_call_timeout(edit_distance_greedy,(row['string_1'],row['string_2']),timeout_time, pool)
                if isinstance(results, int):
                    ed = results
                else:
                    ed = results['ed']
                ...
                if not file_name.endswith('_200.csv'):
                    ...
                    #Pure Recursion
                    algorithm = 'pure_recursion'
                    results, duration_time, duration_clock = meassure_single_call_timeout(RecED,(row['string_1'],row['string_2']),timeout_time, pool)
                    if isinstance(results, int):
                        ed = results
                    else:
                        ed = results[0]
